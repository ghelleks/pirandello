...
- Does it deploy hook scripts to `~/.pirandello/hooks/` from the bundled package data (not from any checkout path)? OODA guard scripts are **not** deployed by `masks setup` (they ship with **`beckett`**).
...
- In `--role-env` mode, does it create role `.env` files from `templates/role.env.example` and seed defaults so they are not empty?
...
- Does it avoid re-copying `.gitignore` if it already exists?
...
- In `--role-env` (or `--interactive`) mode, is `templates/role.env.example` copied to `$BASE/consulting/.env` (only if not already present)?
...
- Does it still run all checks even when some fail (not abort on first failure)?
...
Questions the proposal must answer:
- For each Role, is the Role name, last session commit timestamp, and last remote `HEAD` timestamp shown?
- (OODA log lines belong to **`beckett status`**, not `masks status`.)
...
After `masks add-role foo`, `$BASE/foo/` contains: `Memory/INDEX.md`, `Reference/INDEX.md`, `Archive/INDEX.md`, `.gitignore`, `AGENTS.md`, and `.cursor/hooks.json` referencing `~/.pirandello/hooks/`.  
...
Running `masks setup` a second time on a fully configured system, then running it a third time, produces two sets of `.bak.<timestamp>` files for `AGENTS.md` and hook scripts — one per overwrite. `.env` and `.gitignore` are not touched on re-runs.  
...
**`AGENTS.md` missing or broken.** `$BASE/AGENTS.md` is absent, or a symlink target is missing. Symptom: `masks doctor` fails `agents_global`; agents miss global conventions. Maps to: M-03, M-10.
...
**`masks doctor` only checks the first failing condition.** The command exits early on the first failed check rather than running all checks. Symptom: users see one failure, fix it, re-run doctor, find another — never get a complete health picture in one run. Indicates: early exit on first non-zero check rather than accumulated results. Maps to: M-06.
