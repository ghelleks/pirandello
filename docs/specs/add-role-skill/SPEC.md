...
**Deliverables:** `skills/add-role/SKILL.md`. Invoked by `masks add-role --interactive` and directly by the user.
...
3. The skill must not show the raw `.env.example` file or `.env` file to the user.
...
How each `.env.example` key is presented to the user. The question template (key name + plain-language explanation + where to find it).
...
### 6. Open decisions
What the skill does if `.env.example` is not found. Whether signal source collection writes to OODA.md or returns structured data to `masks add-role` for writing.
...
|---|---|---|
| M-01 | All keys covered | Every key in `.env.example` is asked about; none skipped silently |
...
| M-05 | No file exposure | The skill never shows `.env` or `.env.example` file contents to the user |
