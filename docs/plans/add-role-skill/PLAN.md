...
**Deliverable:** `skills/add-role/SKILL.md` (agent instructions only; no personal content in `pirandello/`)
...
The `add-role` skill completes **interactive** Role setup after `masks add-role <name> --interactive` has already performed all **infrastructure** work: created `$BASE/<name>/`, copied `templates/.gitignore`, copied `templates/OODA.md`, copied `.env.example` → `$BASE/<name>/.env`, initialized git, installed hooks, and seeded `Memory/`, `Reference/`, `Archive/` with `INDEX.md` files.

The skill **owns**:

- Conversational collection of a value (or deliberate blank) for **every** `KEY=` entry in the canonical `pirandello/.env.example`, written into the Role’s `.env` in **the same key order** as the template.
- Optional `**git remote add origin <url>`** (or equivalent safe wiring) when the user supplies a URL.
- Collection of **OODA signal sources** and **merging them into** the Role’s existing `OODA.md` under the **Signal Sources** section so `masks run` and future OODA sessions see an accurate, role-specific list.
...
- **Canonical key list:** Parse `KEY=value` lines from the framework’s `.env.example` (ignore comments and blank lines). That ordered list is the **only** source of which keys to ask about (satisfies M-01 dynamically when keys change).
...
2. **Never** paste or quote the contents of `.env`, `.env.example`, or a bulk list of keys (M-05).
...
- For each key in `.env.example` order: update map, then **rewrite the entire `.env`** with:
  - Same key order as `.env.example`.
...
The committed `pirandello/.env.example` is the source of truth. The shipped `SKILL.md` must include a **catalog** mapping each key to `{human_label, explanation, where_to_find, hint_if_stuck}`. Below is the **initial catalog** to keep in sync with the first committed template (adjust only when `.env.example` changes).
...
| `GWS_ACCOUNT_WORK`     | work Google account id       | Ask which Google account label Pirandello should use for **work** calendar and mail tools. | The short name you use in Pirandello’s Google CLI configuration.                            | Offer an example: “often `work` or your company email prefix” — without naming a real person.                                |
| `GWS_ACCOUNT_PERSONAL` | personal Google account id   | Ask which Google account label to use for **personal** calendar and mail.                  | Your personal account alias in the same tooling.                                            | Example: “often `personal`”.                                                                                                 |
| `GMAIL_REFRESH_TOKEN`  | Gmail access (refresh token) | Ask for the refresh token Pirandello uses to read mail on behalf of this role.             | From your Google OAuth / Pirandello mail setup flow.                                        | One sentence: run the existing token helper from your setup doc, or skip if this role does not use Gmail.                    |
...
| `FEEDLY_AUTH_TOKEN`    | Feedly token                 | Ask for Feedly auth if this role monitors RSS through Feedly.                              | Feedly developer / account token screens.                                                   | “Skip if you don’t use Feedly here.”                                                                                         |
...
**Note:** If `.env.example` contains keys not yet in the catalog, the implementer **adds a row before release** — the skill must not invent silent omission (M-01).
...
- Preserve the rest of `OODA.md` (Agenda, Excluded, etc.) unchanged so `masks run` and `masks doctor` continue to parse the agenda.

### Why not return structured data to the CLI only

Direct invocation has **no** CLI consumer. Writing `OODA.md` in place keeps **one** code path, satisfies “stored in a way `masks run` can use” (the heartbeat reads `OODA.md` as its spec), and avoids duplicating write logic in Python.
...
| ID   | Result | Note                                                                                   |
| ---- | ------ | -------------------------------------------------------------------------------------- |
| M-01 | Pass   | Parser iterates every `KEY=` from `.env.example`; catalog must cover all shipped keys. |
